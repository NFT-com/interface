  ...
  if (getEnvBool(Doppler.NEXT_PUBLIC_HOMEPAGE_V3_ENABLED)) {
    return <div className="overflow-x-auto relative w-full">
      <table className="border-x-0 w-full min-w-[700px]">
        <thead>
          <tr className="text-body leading-body font-body text-[#FDB720] uppercase">
            <th scope="col" className='text-left minmd:text-body text-sm leading-body font-body'>Profile</th>
            <th scope="col" className='text-left minmd:text-body text-sm leading-body font-body'>Number of Genesis Keys</th>
            <th scope="col" className='text-left pr-3 minmd:text-body text-sm leading-body font-body'>Number of NFT Collections</th>
            <th scope="col" className='text-left pr-3 minmd:text-body text-sm leading-body font-body'>Items Collected</th>
          </tr>
        </thead>
        <tbody className="bg-always-white">
          {data?.leaderboard?.items.map((item, i) => (
            ...
            >
              <td className='group-hover:text-[#FF9B37] group-hover:bg-[#FFF0CB]'>
                <Link href={item.url}>
              ...
              <td className='group-hover:bg-[#FFF0CB]'>
                <div className={tw('flex items-end minmd:text-body text-sm leading-body font-bold',
                  'whitespace-nowrap text-right')}>
                  <span className={tw(
                    'bg-key-bg group-hover:bg-[#FF9B37] rounded-full flex items-center py-1 px-2',
                    'text-white group-hover:text-white'
                  )}>
                    <svg className='mr-1' width="22" height="14" viewBox="0 0 22 14" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <table className="min-w-full border-x-0">
        <thead>
          <tr className="text-body leading-body font-body text-[#7F7F7F]">
            ...
            <th scope="col" className='text-right pr-3 minmd:text-body text-sm leading-body font-body'>Number of NFT Collections</th>
            <th scope="col" className='text-right minmd:text-body text-sm leading-body font-body'>Number of Genesis Keys</th>
          </tr>
        </thead>
        <tbody className="bg-always-white">
          {data?.leaderboard?.items.map((item, i) => (
