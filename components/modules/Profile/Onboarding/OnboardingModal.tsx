...
export default function OnboardingModal({ profileURI, onClose }: OnboardingModalProps) {
  ...
  const usersActions = profileData?.profile?.usersActionsWithPoints.find(
    user => user?.userId === profileData?.profile?.ownerUserId
  );
  ...
  const onboardingItems = useMemo(
    () => [
      ...
      },
      {
        ...
        coins: 10,
        description:
          'Refer friends to join you on NFT.com. Once your referral creates a profile, this step will be marked as completed.',
        buttonText: 'Continue'
      },
      {
        name: 'Buy NFTs',
  ...
  return (
    <>
      <div
        ...
        className={tw('fixed', expanded && 'inset-0 bg-black bg-opacity-25 minlg:bg-opacity-0')}
      />
      <div
        className={tw(
          'fixed bottom-0 right-[50%] z-[105] h-max min-h-max min-w-[375px] translate-x-1/2 overflow-y-auto minlg:absolute minlg:right-4 minlg:top-20 minlg:z-[103] minlg:translate-x-0',
          expanded && 'top-[10%] minlg:right-4 minlg:top-20'
        )}
      >
        <div className='flex min-h-max items-start justify-end p-4 text-center'>
          <div>
            <div
              className={tw(
                'w-[342px] rounded-lg minlg:w-[330px]',
            ...
            >
              <div className='px-7'>
                <div className='flex items-center justify-between font-medium'>
                  <h3 className='text-[24px] leading-6 text-black'>Get Started</h3>
                  {getEnvBool(Doppler.NEXT_PUBLIC_PROFILE_POINTS_ENABLED) && (
                    <div className='flex items-center rounded-full bg-[#FFF4CA] py-1 pl-1 pr-4'>
                      <div className='mr-[5px] h-[24px] w-[24px] minmd:h-[34px] minmd:w-[34px]'>
                        <BlurImage alt='default profile photo' src='/assets/nft_profile_default.webp' fill localImage />
                      </div>
                      {profileData?.profile?.usersActionsWithPoints[0]?.totalPoints || 5}/
                      <span className='text-[#6A6A6A]'>{totalPoints || '-'}</span>
                    </div>
                  )}
                  <X onClick={() => onClose()} className='hover:cursor-pointer' size={32} color='black' weight='bold' />
                </div>
                <p className='mt-3 w-11/12 font-medium'>
                  Complete these steps to set up your{' '}
                  <span className='bg-gradient-to-r from-[#FAC213] to-[#FF9B37] bg-clip-text text-transparent'>
                    NFT Profile
                  </span>
                </p>

                <div className='mt-7'>
                  <div className={tw('flex items-center justify-between', expanded && 'mb-7')}>
                    <div className='h-3 w-[85%] rounded-full bg-[#E6E6E6]'>
                      <div
                        style={{
                          width: `${
                            totalPoints
                              ? Math.floor(
                                  ((profileData?.profile?.usersActionsWithPoints[0]?.action.length || 5) /
                        ...
                        className={tw('h-3 rounded-full bg-[#26AA73]')}
                      ></div>
                    </div>
                ...
                <>
                  <div className='border-t px-4 pt-3'>
                    <div className='flex flex-col space-y-2'>
                      <OnboardingModalItem items={onboardingItems} />
                    </div>
                  </div>

                  {getEnvBool(Doppler.NEXT_PUBLIC_PROFILE_POINTS_ENABLED) && (
                    <div className='mt-3 px-[55px]'>
                      <button
                        type='button'
                        className={tw(
                          'inline-flex w-full items-center justify-center',
                          'rounded-xl border border-transparent bg-[#F9D54C] hover:bg-[#EFC71E]',
                          'py-2 font-medium text-black',
