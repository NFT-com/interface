...
export function NftGrid(props: NftGridProps) {
  const { toggleHidden, editMode, draftNftsDescriptionsVisible, draftLayoutType } = useContext(ProfileContext);
  ...
  return (
    <div
      className={tw(
        'grid w-full',
        'mt-4 gap-4 minlg:mt-0',
        (draftLayoutType ?? savedLayoutType) === 'Default'
          ? 'grid-cols-2 minmd:grid-cols-2 minlg:grid-cols-3 minxl:grid-cols-4 minxxl:grid-cols-5'
          : '',
        (draftLayoutType ?? savedLayoutType) === 'Mosaic'
          ? 'grid-cols-2 minmd:grid-cols-3 minlg:grid-cols-4 minxl:grid-cols-6'
          : '',
      ...
      {items?.map((nft: PartialDeep<DetailedNft>, index) => (
        ...
        >
          <div
            className={tw(
              'NFTCardContainer',
              'mb-2 flex justify-center minmd:mb-0',
              (draftLayoutType ?? savedLayoutType) === 'Default' ? 'mb-10' : '',
              (draftLayoutType ?? savedLayoutType) === 'Featured'
                ? `${[0, 1, 2].includes(index) ? 'col-span-2 row-span-2' : 'col-span-1'} mb-10`
                : '',
              (draftLayoutType ?? savedLayoutType) === 'Mosaic' && screenWidth > 1199
                ? `${index % 7 === 0 ? 'col-span-3 row-span-3' : ''} ${
                    (index - 4) % 7 === 0 ? 'col-span-2 row-span-2' : ''
                  }`
                : '',
              (draftLayoutType ?? savedLayoutType) === 'Mosaic' && screenWidth > 900 && screenWidth <= 1199
                ? `${mosaicArray2.includes(index) ? 'col-span-2 row-span-2' : ''}`
                : '',
              (draftLayoutType ?? savedLayoutType) === 'Mosaic' && screenWidth > 600 && screenWidth <= 899
                ? `${mosaicArray.includes(index) ? 'col-span-2 row-span-2' : ''}`
                : '',
              (draftLayoutType ?? savedLayoutType) === 'Spotlight'
                ? 'col-span-2 col-start-2 mb-4 minlg:col-span-4 minlg:col-start-3'
                : ''
            )}
          >
            <NFTCard
              ...
              visible={editMode ? !nft?.hidden : null}
              onVisibleToggle={() => {
                if (editMode) {
