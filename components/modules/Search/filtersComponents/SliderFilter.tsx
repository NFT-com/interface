...
export const SliderFilter = (props: SliderFilterProps) => {
  ...
  leftTooltip.innerHTML = checkSalesValue(
    props.defaultValues ? props.defaultValues[0].toString() : props.min.toString()
  );
  rightTooltip.innerHTML = checkSalesValue(
    props.defaultValues ? props.defaultValues[1].toString() : props.max.toString()
  );
  leftTooltip.classList.add(
    'z-2',
    'text-sm',
    'px-[10px]',
    'rounded-[16px]',
  ...
  return (
    <div className='flex flex-col'>
      <Slider
        ...
        handleStyle={[
          { backgroundColor: 'black', border: 'none', width: '18px', height: '18px' },
          { backgroundColor: 'black', border: 'none', width: '18px', height: '18px' }
        ]}
        onChange={value => setValue([value[0], value[1]])}
        onAfterChange={value => props.click(value)}
      />
      <div className='color-[#6A6A6A] noi-grotesk relative z-[-1] mt-4 flex flex-row items-center justify-between text-xs'>
