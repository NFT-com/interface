...
export default function MintGKProfileCard() {
  ...
  const { data: mintedProfiles, loading: loadingMintedProfiles } = useProfilesMintedByGKQuery(
    selectedGK?.tokenId.toString(),
    defaultChainId
  );
  ...
  useEffect(() => {
    if (!isLoaded) {
      setIsLoaded(
        !loadingMintedProfiles && !loadingClaimable && !loadingMintedProfiles && !loadingGKTokens && claimable != null
      );
    }
    ...
    <>
        ...
        <LoadedContainer newLoader fitToParent loaded={isLoaded}>
          {currentAddress && claimable?.length === 0 && (
            <div className='flex flex-col items-center justify-center'>
              <p className='mb-2 mt-10 text-red-500'>No Genesis Key detected for mint.</p>
          ...
          <div className='mt-9'>
            ...
            {currentAddress &&
              ...
              mintedProfiles?.profilesMintedByGK.map(profile => (
                <Link key={profile.url} href={`/${profile.url}`}>
                  <div
                    className='mb-4 flex h-14 items-center justify-between rounded-xl bg-[#FCE795] px-4 py-2 hover:cursor-pointer'
                    key={profile.url}
                  >
            ...
            {loadingMintedProfiles && currentAddress && (
              <>
                <div className='mb-4 flex h-14 animate-pulse items-center justify-between rounded-xl bg-gray-300 px-4 py-2'></div>
                <div className='mb-4 flex h-14 animate-pulse items-center justify-between rounded-xl bg-gray-300 px-4 py-2'></div>
                <div className='mb-4 flex h-14 animate-pulse items-center justify-between rounded-xl bg-gray-300 px-4 py-2'></div>
                <div className='mb-4 flex h-14 animate-pulse items-center justify-between rounded-xl bg-gray-300 px-4 py-2'></div>
              </>
            ...
            <div className='flex items-center justify-between'>
              <div>
                <DropdownPicker
                  ...
                  selectedIndex={0}
                  options={filterNulls(getGKOptions())}
                  showKeyIcon
                />
              </div>
              {selectedGK &&
              inputCount <= 4 &&
              selectedGK?.claimable > 0 &&
              (selectedGK?.claimable ?? inputCount) - inputCount !== 0 ? (
                <p className='hover:cursor-pointer' onClick={() => setInputVisible()}>
                  Add NFT Profile
                </p>
              ) : null}
            </div>
          ...
          <div className='mt-12 minlg:mt-[59px]'>
            {!isNullOrEmpty(claimable) && (
              <p className='mb-3 text-center font-normal text-[#5B5B5B]'>
                {selectedGK && selectedGK?.claimable <= 3
                  ? `Minted ${4 - (selectedGK?.claimable ?? 0)} `
                  : `Minting ${inputCount} `}
      ...
      {modalOpen && (
        <DynamicMintProfileModal
          ...
          profilesToMint={filteredInputs}
          gkTokenId={selectedGK?.tokenId}
          type='GK'
        />
      )}
    </>
