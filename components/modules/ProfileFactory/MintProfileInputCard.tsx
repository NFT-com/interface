...
export default function RemoveModal({ currentURI, setCurrentURI, setMintModalOpen, minting, setMinting }: MintProfileInputCardProps) {
  ...
  return (
    <div className='relative mt-28 minlg:mt-12 z-50 px-5'>
      <div className='max-w-[600px] mx-auto bg-white rounded-[20px] pt-6 minmd:pt-[64px] px-4 minmd:px-12 minlg:px-[76px] pb-10 font-medium'>
        ...
        {dropdownValue === 'Free' ? <p className='mt-6 text-xl w-5/6'>Every wallet receives one <span className='text-secondary-yellow'>free mint!</span></p> : null}
        {dropdownValue === 'Paid' ? <p className='mt-6 text-xl w-5/6'>You have already received one free mint</p> : null}
        {dropdownValue !== 'Paid' && dropdownValue !== 'Free' ? <p className='mt-6 text-xl w-5/6'>Genesis Key holders receive <span className='text-secondary-yellow'>four free mints!</span></p> : null}
        <MintProfileInputField listings={listings} currentURI={currentURI} minting={minting} setCurrentURI={setCurrentURI} loadingTokenId={loadingTokenId} getProfileStatus={getProfileStatus} />
        <div className='w-1/2'>
          <DropdownPicker
