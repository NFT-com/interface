...
export default function ConnectedAccounts({ selectedProfile, associatedAddresses }: ConnectedAccountsProps) {
  ...
  const submitHandler = async (input?: string) => {
    ...
    if (deniedEvent) {
      updateHideIgnored({ hideIgnored: false, eventIdArray: [deniedEvent.id] }).then(() => {
        mutateHidden();
  ...
  useEffect(() => {
    if (
      associatedAddresses?.pending.find(element => element.chainAddr === inputVal) ||
      associatedAddresses?.accepted.find(element => element.chainAddr === inputVal)
    ) {
  ...
  return (
    <div id='addresses' className='font-noi-grotesk'>
      <h2 className='mb-2 text-2xl font-bold tracking-wide text-black'>Associate Addresses</h2>
      <p className='mb-4 text-blog-text-reskin'>
        The NFTs contained or collections deployed by associated Ethereum addresses will display on this NFT Profile.
      </p>

      <SettingsForm
        buttonText='Request Association'
        submitHandler={openModal}
        {...{ inputVal, isAssociatedOrPending }}
        changeHandler={setInputVal}
      />

      {associatedAddresses?.accepted?.length ||
      associatedAddresses?.pending?.length ||
      associatedAddresses?.denied?.length ? (
        <div className='mt-6 w-full'>
          <p className='mb-2 pl-1 font-semibold text-blog-text-reskin'>Associated Addresses</p>
          <div className='mb-1 flex  justify-between p-1'>
            <p className='mr-[3.1rem] w-1/2 text-sm text-blog-text-reskin'>Address</p>
            <p className='w-1/2 text-sm text-blog-text-reskin'>Network</p>
          </div>
          ...
          {associatedAddresses?.denied?.map(
            address =>
              !address.hideIgnored && (
                <AssociatedAddress
                  {...{ selectedProfile }}
