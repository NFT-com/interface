...
import {
  ...
  convertDurationToSecForNumbersOnly,
  filterValidListings,
  SaleDuration
} from 'utils/marketplaceUtils';
import {
  createNativeParametersForNFTListing,
  onchainAuctionTypeToGqlAuctionType,
  UnsignedOrder
} from 'utils/nativeMarketplaceHelpers';
...
export interface NFTListingsContextType {
  ...
  clear: () => void;
  listAll: (itemsToList: StagedListing[], marketplace?: ExternalProtocol) => Promise<ListAllResult>;
  ...
  toggleTargetMarketplace: (
    ...
    previousSelectedMarketplace?: ExternalProtocol
  ) => void;
  ...
  setAllListingsFail: (value: boolean) => void;
}
...
export const NFTListingsContext = React.createContext<NFTListingsContextType>({
  ...
  allListingsFail: false,
  setAllListingsFail: () => null
});

/**
 * This context provides state management and helper functions for the NFT listings cart.
 */
export default function NFTListingsContextProvider(props: PropsWithChildren<any>) {
  const [toList, setToList] = useState<Array<StagedListing>>([]);
  const [submitting, setSubmitting] = useState(false);
  const [sidebarVisible, setSidebarVisible] = useState(false);
  // If all listings fail, this toggles the error messaging on listing page
  const [allListingsFail, setAllListingsFail] = useState(false);
  ...
  const decreasingPriceErrorEnabled = useCallback((decreasingPriceInvalidInputs: boolean) => {
    setDecreasingPriceError(decreasingPriceInvalidInputs);
  }, []);
  ...
  const allListingsConfigured = useCallback(() => {
    const unconfiguredNft = toList.find((stagedNft: StagedListing) => {
      const lowestX2Y2Listing = getLowestPriceListing(
        filterValidListings(stagedNft?.nft?.listings?.items),
        ethPriceUSD,
        defaultChainId,
      ...
      const hasX2Y2LowerListing =
        parseInt((lowestX2Y2Listing?.order?.protocolData as X2Y2ProtocolData)?.price) <
          Number(
            stagedNft?.targets?.find(target => target.protocol === ExternalProtocol.X2Y2) && stagedNft?.startingPrice
          ) ||
        parseInt((lowestX2Y2Listing?.order?.protocolData as X2Y2ProtocolData)?.price) <
          Number(stagedNft?.targets?.find(target => target.protocol === ExternalProtocol.X2Y2)?.startingPrice);
      const hasLooksrareLowerListing =
        parseInt((lowestLooksrareListing?.order?.protocolData as LooksrareProtocolData)?.price) <
          Number(
            stagedNft?.targets?.find(target => target.protocol === ExternalProtocol.LooksRare) &&
              stagedNft?.startingPrice
          ) ||
        parseInt((lowestLooksrareListing?.order?.protocolData as LooksrareProtocolData)?.price) <
          Number(stagedNft?.targets?.find(target => target.protocol === ExternalProtocol.LooksRare)?.startingPrice);
      const hasNftcomLowerListing =
        parseInt((lowestNftcomListing?.order?.protocolData as NftcomProtocolData)?.takeAsset[0].value) <
          Number(
            stagedNft?.targets?.find(target => target.protocol === ExternalProtocol.NFTCOM) && stagedNft?.startingPrice
          ) ||
        parseInt((lowestNftcomListing?.order?.protocolData as NftcomProtocolData)?.takeAsset[0].value) <
          Number(stagedNft?.targets?.find(target => target.protocol === ExternalProtocol.NFTCOM)?.startingPrice);

      if (hasX2Y2LowerListing || hasLooksrareLowerListing || hasNftcomLowerListing) {
        return true;
      }
      const hasGeneralConfig =
        stagedNft.startingPrice != null &&
      ...
      const unconfiguredTarget = stagedNft.targets.find(target => {
        const decreasingPriceInvalidInputs =
          target.protocol === ExternalProtocol.NFTCOM &&
          target.auctionType === 2 &&
          (target.endingPrice == null ||
            BigNumber.from(target.endingPrice).eq(0) ||
            (target.endingPrice && target.startingPrice && Number(target.startingPrice) <= Number(target.endingPrice)));
        decreasingPriceErrorEnabled(decreasingPriceInvalidInputs);

        const englishAuctionPriceinvalidInputs =
          target.protocol === ExternalProtocol.NFTCOM &&
          target.auctionType === 1 &&
          (target.buyNowPrice == null ||
            BigNumber.from(target.buyNowPrice).eq(0) ||
        ...
        return (
          target.startingPrice == null ||
          BigNumber.from(target.startingPrice).eq(0) ||
          (target.duration ?? stagedNft.duration) == null ||
          isNullOrEmpty(target.currency) ||
          (target.protocol === ExternalProtocol.NFTCOM && target.auctionType == null) ||
          decreasingPriceInvalidInputs ||
          englishAuctionPriceinvalidInputs
        );
      });
  ...
  const toggleTargetMarketplace = useCallback(
    (
      targetMarketplace: ExternalProtocol,
      toggleListing?: PartialDeep<StagedListing>,
      previousSelectedMarketplace?: ExternalProtocol
    ) => {
      const targetFullyEnabled =
        toList.find(nft => {
      ...
      if (toggleListing) {
        ...
        setToList(
          toList.slice().map(stagedNft => {
            if (toggleListing?.nft?.id === stagedNft?.nft?.id) {
              if (previousSelectedMarketplace) {
                  ...
                  targets: stagedNft.targets.map(target => {
                    if (target.protocol === previousSelectedMarketplace) {
                      return {
                        ...
                        currency:
                          stagedNft.currency ?? targetMarketplace === ExternalProtocol.X2Y2
                            ? supportedCurrencyData.ETH.contract
              ...
              return {
                ...stagedNft,
                targets:
                  stagedNft.targets.find(target => target.protocol === targetMarketplace) != null
                ...
                stagedNft.targets?.find(target => target.protocol === targetMarketplace) != null
                  ...
                  : [
                      ...(stagedNft.targets ?? []),
                      {
                        ...
                        currency:
                          stagedNft.currency ??
                          (targetMarketplace === ExternalProtocol.X2Y2 || targetMarketplace === ExternalProtocol.NFTCOM)
                            ? supportedCurrencyData.ETH.contract
                            : supportedCurrencyData.WETH.contract,
                        listingError: false
                      }
          ...
          return {
            ...
            duration:
              typeof duration === 'number'
                ? convertDurationToSecForNumbersOnly(duration)
                : convertDurationToSec(duration),
  ...
  const setCurrency = useCallback(
    (listing: PartialDeep<StagedListing>, currency: SupportedCurrency, targetProtocol?: ExternalProtocol) => {
      setToList(
        toList.slice().map(stagedNft => {
          if (listing?.nft?.id === stagedNft.nft?.id) {
            return {
  ...
  const setTypeOfAuction = useCallback(
    (listing: PartialDeep<StagedListing>, auctionType: number, targetProtocol?: ExternalProtocol) => {
      setToList(
        toList.slice().map(stagedNft => {
          if (listing?.nft?.id === stagedNft.nft?.id) {
            return {
  ...
  const setHasListingError = useCallback(
    (listing: PartialDeep<StagedListing>, hasError: boolean, targetProtocol?: ExternalProtocol) => {
      setToList(
        toList.slice().map(stagedNft => {
          if (listing?.nft?.id === stagedNft.nft?.id) {
            return {
  ...
  const getTarget = useCallback(
    (listing: PartialDeep<StagedListing>, protocol: ExternalProtocol) => {
      return toList?.find(l => l.nft.id === listing.nft.id)?.targets?.find(target => target.protocol === protocol);
    },
    [toList]
  );

  const prepareListings = useCallback(
    async (nonceOverride?: number) => {
      ...
      const preparedListings = await Promise.all(
        toList.map(async stagedNft => {
          const preparedTargets = await Promise.all(
            stagedNft.targets?.map(async target => {
              ...
              if (target.protocol === ExternalProtocol.X2Y2) {
                return target;
              }
              if (target.protocol === ExternalProtocol.NFTCOM) {
              ...
              const collectionFee: Fee =
                contract?.payout_address && contract?.dev_seller_fee_basis_points
                  ? {
  ...
  const listAll: (itemsToList: StagedListing[], marketplace?: ExternalProtocol) => Promise<ListAllResult> = useCallback(
    async (itemsToList, marketplace) => {
      setSubmitting(true);
      const results = await Promise.all(
        itemsToList.map(async (listing: StagedListing) => {
          const results = await Promise.all(
            listing.targets?.map(async (target: ListingTarget) => {
              if (
                isNullOrEmpty(marketplace)
                  ? target.protocol === ExternalProtocol.LooksRare
                  : target.protocol === ExternalProtocol.LooksRare && marketplace === ExternalProtocol.LooksRare
              ) {
              ...
              if (
                isNullOrEmpty(marketplace)
                  ...
                  : target.protocol === ExternalProtocol.X2Y2 && marketplace === ExternalProtocol.X2Y2
              ) {
                const order: X2Y2Order = await createX2Y2ParametersForNFTListing(
                  ...
                  !isNullOrEmpty(listing?.startingPrice?.toString())
                    ? listing.startingPrice.toString()
                    : target.startingPrice.toString(),
              ...
              if (
                isNullOrEmpty(marketplace)
                  ? target.protocol === ExternalProtocol.NFTCOM
                  : target.protocol === ExternalProtocol.NFTCOM && marketplace === ExternalProtocol.NFTCOM
              ) {
              ...
              if (
                isNullOrEmpty(marketplace)
                  ...
                  : target.protocol === ExternalProtocol.Seaport && marketplace === ExternalProtocol.Seaport
              ) {
                ...
                const result = await listNftSeaport(signature, {
                  ...target.seaportParameters,
                  counter: seaportCounter
                });
                if (!result) {
                  setHasListingError(listing, true, ExternalProtocol.Seaport);
                  return ListAllResult.ApiError;
          ...
          return (results.includes(ListAllResult.ApiError) || results.includes(ListAllResult.SignatureRejected)) &&
            results.includes(ListAllResult.Success)
            ? ListAllResult.PartialError
            : results.includes(ListAllResult.SignatureRejected)
      ...
      return ((results.includes(ListAllResult.ApiError) || results.includes(ListAllResult.SignatureRejected)) &&
        results.includes(ListAllResult.Success)) ||
        results.includes(ListAllResult.PartialError)
        ? ListAllResult.PartialError
        : results.includes(ListAllResult.SignatureRejected)
        ? ListAllResult.SignatureRejected
        : results.includes(ListAllResult.ApiError)
  ...
  const approveCollection = useCallback(
    async (listing: StagedListing, target: ExternalProtocol) => {
  ...
  return (
    ...
    >
      {sidebarVisible && <NFTCartSidebar selectedTab={selectedTab} onChangeTab={setSelectedTab} />}
      {props.children}
    </NFTListingsContext.Provider>
  );
}
