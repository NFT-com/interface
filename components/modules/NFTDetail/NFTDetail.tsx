                    ...
                    >
                      <span className="text-base font-medium leading-5 font-noi-grotesk text-link">
                        {!profileOwnerToShow?.url == null ?
                          shortenAddress(props.nft?.wallet?.address ?? props.nft?.owner, 0) :
                          profileOwnerToShow?.url
                        }
                      </span>
                    </div> :
                    <Link href={getEtherscanLink(Number(defaultChainId), props.nft?.wallet?.address ?? props.nft?.owner, 'address')}>
                      <span className="text-[#1F2127] text-base cursor-pointer hover:underline font-medium leading-5 font-noi-grotesk pl-3">
                        {shortenAddress(props.nft?.wallet?.address ?? props.nft?.owner, isMobile ? 4 : 6) ?? 'Unknown'}
                      </span>
                    </Link>
                }
              </div>
