...
function ExternalListingTile(props: ExternalListingTileProps) {
  ...
  const nftInPurchaseCart = useMemo(() => {
    return (
      toBuy?.find(
        purchase => purchase.nft?.id === props.nft?.id && purchase?.orderHash === props?.listing?.order?.orderHash
      ) != null
    );
  }, [props?.listing?.order?.orderHash, props.nft?.id, toBuy]);

  const { mutate: mutateNft } = useNftQuery(props?.nft?.contract, props?.nft?.tokenId);
  ...
  const getButton = useCallback(
    (type: ListingButtonType) => {
      let listedCurrency;
      if (
        (listing?.order?.protocol as ExternalProtocol) === ExternalProtocol.X2Y2 ||
        (listing?.order?.protocol as ExternalProtocol) === ExternalProtocol.LooksRare
      ) {
            ...
            <Button
              ...
              onClick={() => {
                stageListing({
                  ...
                  openOrderId:
                    (listing?.order?.protocol as ExternalProtocol) === ExternalProtocol.X2Y2
                      ? (listing?.order?.protocolData as X2Y2ProtocolData).id
                      : null,
                  targets: [
                    {
            ...
            <Button
              ...
              onClick={async () => {
                ...
                if (listingProtocol === ExternalProtocol.LooksRare) {
                  ...
                  const result = await cancelNftcomListing(listing, NftcomExchange);
                  if (result) {
                    updateActivityStatus([listing?.id], ActivityStatus.Cancelled);
                    mutateNft();
                  }
          ...
          return (
            <Button
              ...
              onClick={async () => {
                ...
                const allowance = await currencyData.allowance(
                  currentAddress,
                ...
                stagePurchase({
                  ...
                  protocolData:
                    listingProtocol === ExternalProtocol.Seaport
  ...
  if (
    ![ExternalProtocol.LooksRare, ExternalProtocol.Seaport, ExternalProtocol.X2Y2, ExternalProtocol.NFTCOM].includes(
      listingProtocol as ExternalProtocol
    )
  ) {
  ...
  return (
    <div className='relative my-6 flex flex-col rounded-[10px] bg-[#F8F8F8] pt-12 font-noi-grotesk'>
      <div className='absolute top-0 flex h-8 w-full items-center rounded-t-[10px] bg-[#FCF1CD] pl-6'>
        <span className='font-bold text-secondary-txt'>
          {listing?.order?.protocol === ExternalProtocol.NFTCOM
            ? `${getAuctionTypeDisplayName((listing?.order?.protocolData as NftcomProtocolData).auctionType)}`
            : 'Fixed Price'}
        </span>
      </div>
      <div className='mb-4 flex items-center px-4'>
        ...
        >
          {listing?.order?.exchange === ExternalExchange.X2Y2 ? (
            <X2Y2Icon className='relative mx-1.5 h-9 w-9 shrink-0' alt='X2Y2 logo' layout='fill' />
          ) : listing?.order?.exchange === ExternalExchange.NFTCOM ? (
            <NFTLogo className='relative mx-1.5 h-9 w-9 shrink-0' alt='NFT.com logo' layout='fill' />
            ...
            <div
              ...
              className='relative flex h-6 w-6 shrink-0 hover:cursor-pointer'
            >
        ...
        <div className='ml-3 flex flex-col text-primary-txt dark:text-primary-txt-dk'>
          <span className='text-sm text-secondary-txt'>
            Current price on{' '}
            <span className='font-bold'>{getProtocolDisplayName(listing?.order?.exchange as ExternalProtocol)}</span>
          </span>
      ...
      <div className='flex flex-col items-center'>
        {props.buttons?.map(buttonType => {
          return (
            <div className='mb-4 mt-2 flex w-full grow basis-1 items-center px-2' key={buttonType}>
