...
export default function NotificationContextProvider(props: PropsWithChildren) {
  ...
  const { data: expiredListings } = useExpiredNotificationsQuery(currentAddress, defaultChainId);
  ...
  const setUserNotificationActive = useCallback(
    (notification: keyof UserNotifications, notificationValue: boolean) => {
      setNotifications({
        ...notifications,
  ...
  const fetchAssociatedProfiles = useCallback(async () => {
    if (!currentAddress) {
      return null;
    }
    return nftResolver.getApprovedEvm(currentAddress).catch(() => null);
  ...
  useEffect(() => {
    if (!isNullOrEmpty(pendingAssociatedProfiles?.getMyPendingAssociations) && acceptedAssociatedProfiles !== null) {
      const filterAccepted = pendingAssociatedProfiles?.getMyPendingAssociations?.filter(
        a => !acceptedAssociatedProfiles?.some(b => a.url === b.profileUrl)
      );
  ...
  useEffect(() => {
    ...
    if (
      (isNullOrEmpty(pendingAssociationCount) || pendingAssociationCount === 0) &&
      notifications.hasPendingAssociatedProfiles
    ) {
    ...
    if (
      ...
      !notifications.profileNeedsCustomization
    ) {
    ...
    addedAssociatedNotifClicked,
    hasUnclaimedProfiles,
    pendingAssociatedProfiles,
    profileCustomizationStatus,
    removedAssociationNotifClicked,
