...
function Tooltip(props: PropsWithChildren<ToolTipProps>) {
  ...
  const setContainerPosition = (orientation, customLeftPosition, customFullLeftPosition) => {
    ...
    switch (orientation) {
      ...
      case orientations.custom:
        classnames = `bottom-full ${
          customFullLeftPosition ?? `left-[${customLeftPosition}%]`
        } translate-x-[-50%] -translate-y-2`;
        break;
  ...
  )} bg-black text-white text-sm p-2 rounded-xl flex items-center transition-all duration-150 ${
    props.tooltipClick ? 'cursor-pointer' : 'pointer-events-none'
  }`;

  const pointerClasses = `bg-black h-3 w-3 absolute z-10 ${setPointerPosition(props.orientation)} rotate-45 ${
    props.tooltipClick ? 'cursor-pointer' : 'pointer-events-none'
  }`;
  ...
  return (
    <div
      className={tw(
        `relative z-50 flex h-full items-center ${props.tooltipClick ? 'cursor-pointer' : ''}`,
    ...
    >
      {props.tooltipComponent ? (
        <div onClick={() => props.tooltipClick()} className={classContainer} style={{ opacity }}>
