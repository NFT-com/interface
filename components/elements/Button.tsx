  ...
  return (
    <button
      className={tw(
        ...
        props?.disabled ? '' : props.scaleOnHover && 'minlg:transform minlg:hover:scale-105',
    ...
    >
      {props?.loading ? (
        <div className='flex w-full items-center justify-center'>
          {getLoader()}
          {props?.loadingText && <span className='ml-2 flex whitespace-normal'>{props?.loadingText}</span>}
        </div>
      ) : (
