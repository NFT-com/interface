      ...
      {(() => {
        if (!isConnected || !currentAddress) {
          return (
            <div>
              <button
                className='flex h-10 w-10 cursor-pointer items-center justify-center rounded-full bg-yellow-300 minlg:hidden'
                onClick={() => {
                  openConnectModal();
                }}
              >
              ...
              <button
                ...
                className={tw(
                  ...
                  `${
                    props?.headerButtonColor
                      ? props.bgLight && props.header
                        ? 'bg-[#F9D54C]'
                        : 'bg-black'
                      : 'bg-[#F9D963]'
                  }`,
                  !props.bgLight && props.header && 'border border-[#6F6F6F]',
                  'rounded-3xl',
                  props.header ? (props.bgLight ? 'text-[#000000]' : 'text-white') : 'text-[#4d4412]',
                  'hidden cursor-pointer flex-row items-center hover:bg-[#EFC71E] minlg:flex',
        ...
        if (chain.unsupported) {
          return (
            <button
              className={tw(
                'block rounded-full font-medium text-black',
                'bg-[#F9D54C]',
        ...
        return <SignedInProfileButtonDropdown />;
      })()}
    </div>
  );
};
