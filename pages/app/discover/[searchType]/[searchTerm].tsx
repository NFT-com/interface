...
export default function ResultsPage({ data }: ResultsPageProps) {
    ...
    if(newFiltersEnabled){
      ...
      if(searchType?.toString() !== 'collections'){
        if(getEnvBool(Doppler.NEXT_PUBLIC_TYPESENSE_SETUP_ENABLED)){
          return SearchableFields.FACET_NFTS_INDEX_FIELDS + ',listedFloor,listings.type,listings.currency,traits.rarity';
        }else {
          return SearchableFields.FACET_NFTS_INDEX_FIELDS + ',listedPx,listingType,currency';
        }
      }else {
        if(getEnvBool(Doppler.NEXT_PUBLIC_TYPESENSE_SETUP_ENABLED)){
          return SearchableFields.FACET_COLLECTIONS_INDEX_FIELDS + ',issuance,isOfficial,isCurated';
        }else {
          return SearchableFields.FACET_COLLECTIONS_INDEX_FIELDS + '';
        }
      }
    }
  };
  const checkFilteredBy = () => {
    if(newFiltersEnabled){
      ...
      if(getEnvBool(Doppler.NEXT_PUBLIC_DISCOVER2_PHASE1_ENABLED) && searchType?.toString() === 'profiles'){
        return 'isProfile: [true] && ' + nftsResultsFilterBy;
      }else {
        return nftsResultsFilterBy;
      }
    }
  ...
  const checkQueryBy = () => {
    if(newFiltersEnabled){
      ...
      if(searchType?.toString() !== 'collections' ){
        if(getEnvBool(Doppler.NEXT_PUBLIC_TYPESENSE_SETUP_ENABLED)){
          return SearchableFields.NFTS_INDEX_FIELDS + ',listings.type,listings.currency,listings.marketplace';
        }else {
          return SearchableFields.NFTS_INDEX_FIELDS + ',marketplace,listingType,currency';
        }
      }else {
