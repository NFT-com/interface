  ...
  switch (action) {
    case 'getNonce':
      try {
        ...
        const result = await fetch(`https://api.looksrare.org/api/v1/orders/nonce?address=${address}`, options).then(
          res => res.json()
        );
    ...
    case 'DEPRECATED_listNFT':
      try {
        ...
        const result = await fetch('https://api.looksrare.org/api/v1/orders', options).then(res => res.json());
        res.status(200).json(result);
      } catch (e) {
