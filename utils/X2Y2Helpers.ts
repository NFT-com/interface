...
async function signOrder(signer: ethers.Signer, order: X2Y2Order): Promise<void | null> {
  const orderData: string = ethers.utils.defaultAbiCoder.encode(orderParamTypes, [
    order.salt,
    order.user,
    order.network,
    order.intent,
...
function makeSellOrder(
  ...
  return {
    ...
    intent: INTENT_SELL,
    delegateType: tokenStandard === 'erc1155' ? DELEGATION_TYPE_ERC1155 : DELEGATION_TYPE_ERC721,
...
export async function signOrderForX2Y2(signer: ethers.Signer, order: X2Y2Order): Promise<void> {
...
async function fetchOrderSign(
  ...
  try {
    ...
    royalty && !isNullOrEmpty(royalty?.toString()) && url.searchParams.set('royalty', royalty.toString());
    payback && !isNullOrEmpty(payback?.toString()) && url.searchParams.set('payback', payback.toString());
...
export async function buyOrder(network: Network, accountAddress: string, order: Order): Promise<RunInput | undefined> {
  if (!(order.id && order.price && order.token)) {
...
export async function createX2Y2ParametersForNFTListing(
  ...
  const order: X2Y2Order = makeSellOrder(network, accountAddress, expirationTime, [{ price, data }], tokenStandard);
...
export const getX2Y2Hex = async (
  ...
  try {
    const { contract, created_at, currencyAddress, end_at, id, maker, price, side, status, tokenId, type } =
      protocolData;
    ...
    throw new Error(`error in getX2Y2Hex: ${err}`);
  }
};

export function decodeCancelInput(input: string): CancelInput {
  return ethers.utils.defaultAbiCoder.decode([cancelInputParamType], input)[0] as CancelInput;
}
...
export async function cancelX2Y2Listing(
  ...
  const input: CancelInput = await getCancelInput(accountAddress, OP_CANCEL_OFFER, orderId, signMessage, sign);
...
export const X2Y2BuyNow = async (
  ...
  try {
    ...
    const tx = await X2Y2Exchange.run(runInput, {
      value: order?.price
    });
